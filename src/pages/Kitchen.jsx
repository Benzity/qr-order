...
export default function Kitchen(){
  ...
  async function updateStatus(o, status){
    const { data, error } = await supabase.from('orders').update({ status }).eq('id', o.id).select().single()
    ...
    if(status==='완료' && o.player_id){
      await fetch('/api/send-push', {
        ...
        body: JSON.stringify({
          ...
          title: '주문 준비 완료',
          message: `주문이 준비됐습니다. 테이블 ${o.table_no} 수령대 방문해주세요.`
        })
      })
    }
  }
        ...
        {orders.map(o=>(
          <div key={o.id} style={{border:'1px solid #ddd', padding:12, borderRadius:8}}>
            <div style={{display:'flex', justifyContent:'space-between'}}>
              <b>#{o.id.slice(0,8)} · 테이블 {o.table_no}</b>
            ...
            <div style={{display:'flex', gap:8}}>
              <button disabled={o.status==='접수'} onClick={()=>updateStatus(o,'접수')}>접수</button>
